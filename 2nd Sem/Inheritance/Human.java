...
public class Human {
    ...
    public void attack(Human target) {
        System.out.println("\n"+this.getName() + " VS " + target.getName());
        int dmg = 0;
        if (this.getJob().equalsIgnoreCase("Wizard")) {
            // 1. if the attacker is a wizard the damage should be based on the intelligence
            // of the character+ strength
            dmg = this.getIntg() + this.getStr();

        } else if (this.getJob().equalsIgnoreCase("Samurai")) {
            // 2. if the attacker is samurai the damage dealt will be strength of the
            // attacker x2 but will also deal damage to himself by half of his strength.
            dmg = this.getStr() * 2;
            this.setHp(this.getHp() - (this.getStr() / 2));
        } else if (this.getJob().equalsIgnoreCase("ninja")) {
            // 3. if the attacker is a ninja damage dealt will be character agi + strength
            // of the target.
    ...
    public void takeAttack(int dmg, Human target) {
        // 4. if the target is a ninja there is 50% chance that the ninja takes no
        ...
        if (target.getHp() > 0) {
            if (target.getJob().equalsIgnoreCase("samurai") && target.getHp() == 1) {
